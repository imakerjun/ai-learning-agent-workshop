...
<Callout type="note">
실제 starter는 [imakerjun/my-brain-v1](https://github.com/imakerjun/my-brain-v1)이고, 거기엔 raw 자료를 던지는 `.inbox/` 한 폴더가 더 있습니다(외부 자료·AI 산출물 모두 흡수, `/수집` 커맨드가 한 줄로 던지는 자리). 점(`.`)으로 시작하는 폴더는 `.claude`처럼 **본인이 직접 안 만지고 커맨드가 관리하는 자동화 영역**, prefix 없는 `outbox/`만 매일 손이 가는 사람용 영역. 두 영역 골격은 그대로, [3. 셋팅](/workshop/setup)에서 풉니다.
</Callout>
