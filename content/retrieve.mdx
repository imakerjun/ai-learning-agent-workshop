...
> outbox와 `.ai-wiki/`가 채워졌으면, 이제 매일 아침 그 둘을 함께 읽고 **어제까지의 나를 숫자 3개로 비추고 오늘 한 줄을 같이 던지는** 메일이 본인에게 도착합니다. 요약·정리가 아닙니다. **안 봐도 그만**인 뉴스레터가 아니라, 오늘 한 줄을 끌어내기 위한 약속입니다.

<Callout type="info">
이 페이지가 약속하는 것 — **본인 Gmail로 매일 아침 7시에 메일 1통 도착**. 셋업은 [4. 셋팅](/setup)보다 한 단계 더 무겁습니다(`/schedule` 호출 + 비공개 GitHub 저장소 + Gmail 연결). 자세한 셋업은 [스케줄 거는 법](#스케줄-거는-법--schedule)에서 정리합니다.
</Callout>

## 알림 시점과 커맨드 호출 시점은 다릅니다

먼저 두 가지를 분리합니다. 같이 묶이면 "아침에 못 돌리면 그날 학습은 망함" 패턴이 생깁니다.

| 무엇 | 언제 | 누가 트리거 |
|---|---|---|
| **`/돌아보기` 호출** (인출·메타인지 1줄 생성) | 본인이 호출하고 싶을 때 — PR 한 편 끝낸 뒤·자기 전·일요일 오후. 시간 제약 X | 본인 |
| **`오늘 한 줄` 메일 발송** (어제까지의 outbox·.ai-wiki를 본인에게) | 매일 아침 7시 (KST, 한국 시간). 단순 알림 | `/schedule` |

핵심은 — `/돌아보기`는 **학습 행위**라 본인이 원할 때 돌리고, `오늘 한 줄`은 **알림**이라 아침 7시에 자동으로 옵니다. 어제 자기 전 `/돌아보기`로 만든 outbox 글이 오늘 아침 7시 메일로 본인에게 도착하는 식입니다.
...
마지막 블록은 본인이 오늘 한 줄을 시작하기 위한 첫 손잡이입니다. 예시 — 어제 메일이 이렇게 도착했다고 가정.
